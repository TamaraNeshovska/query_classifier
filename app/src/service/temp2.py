...
#         # Return current latency as both values if log update fails
...
#     if not labels:
#         logger.warning("No classifier output. Returning empty categories and default settings.")
#         default_settings = map_to_settings([], [])
#         default_settings["latency_seconds"] = round(latency, 3)
...
#     except Exception as e:
#         logger.warning(f"Error merging settings: {e}")
#         settings = {}
    
#     try:
#         _, avg_latency = update_latency_log(prompt, latency)
#     except Exception as e:
#         logger.warning(f"Latency log failed: {e}")
#         avg_latency = latency
